...
# [**Cell competition acts as a purifying selection to eliminate cells with mitochondrial defects during early mouse development**](https://pubmed.ncbi.nlm.nih.gov/34253906/)

Для изучения природы клеток, уничтоженных в результате клеточной конкуренции во время раннего эмбриогенеза мыши с помощью одноклеточного РНК-секвенирования (scRNA-seq), ранние эмбрионы мыши были вскрыты в возрасте E5.5 и культивированы в течение ночи в присутствии или отсутствии ингибиторов каспазы. На следующее утро эмбрионы диссоциировали с помощью Accutase и подвергали сортировке на отдельные клетки в 384-луночных планшетах. Затем проводили выделение тотальной РНК с помощью набора для выделения РНК (RNeasy Mini Kit, Qiagen). scRNA-seq проводили по протоколу Smart-seq274. Для подготовки библиотек использовали метод выделения/обогащения полиА с помощью набора Ultra II Kit (NEB).

### Загрузка необходимых библиотек:

**`gam`** – анализ нелинейных зависимостей\
**`scran`**– анализ данных scRNA-seq\
**`ggplot2`** – создание графиков \
**`dynamicTreeCut`** – разбиение дендрограмм на кластеры\
**`umap`** – реализация алгоритма UMAP (Uniform Manifold Approximation and Projection) для снижения размерности данных\
**`ComplexHeatmap`** - построение тепловых карт с аннотациями\
**`gridExtra` и `grid`** – управление макетами и расположением графиков\
**`viridis`** – цветовые палитры для графиков\
**`destiny`** – создание diffusion maps\
**`circlize`** – построение круговых диаграмм \
**`ggforce`** – расширения для `ggplot2`, включающие дополнительные функции для сложных графиков\
**`fpc`** – кластерный анализ\
**`Seurat`** – анализ данных scRNA-seq\
**`scater`** – обработка данных scRNA-seq
...
```{r}
base_path <- "C:/Users/79143/Documents/kursovaya/kursovaya/Input_data_figures"
```

### Дополнительные данные:

1.  data_raw.Rda: матрица необработанных значений для данных последовательности scRNA
2.  data_tpm.Rda: нормализованная (TPM) матрица значений для данных последовательности scRNA
3.  raw_counts_allele_mt.csv: матрица с информацией о количестве четырех возможных аллелей для каждого основания в митохондриальном геноме.
...
# Клетки, удаляемые в ходе раннего эмбриогенеза мыши, имеют особый транскрипционный профиль.

## Расширенные данные. Рис. 1 (a). Критерии отбора для контроля качества клеток. 

Была проведена количественная оценка транскриптов в данных scRNA-seq, с помощью Salmon v0.8.275 в режиме, основанном на квазикартировании (quasi-mapping-based mode). Сначала был создан индекс транскриптома на основе референсных последовательностей мыши (версия GRCm38.p4) и ERCC spike-in последовательностей. Затем этап количественной оценки был выполнен с помощью функции `quant`, с корректировкой на специфические для последовательностей смещения (флаг `--seqBias`) и на смещения уровня GC содержимого фрагментов (флаг `--gcBias`). Использовалась нормализация TPM (transcript per million).

На полученной матрице сырых данных, включающей 1495 клеток, был проведен контроль качества. Всего 723 клетки прошли контроль качества и были отобраны для последующего анализа. \
\
[Ниже приведены параметры, которые были рассчитаны для каждой клетки:]{.underline}\
**- Слева сверху (numAbove10rpm):** количество генов с уровнем экспрессии выше 10 прочтений на миллион (RPM) равно или превышает 3000.**\
- По центру сверху (Fraction of spikes):** количество ридов, сопоставленных с ERCC-spike-in, деленное на общую сумму обработанных ридов.**\
- Справа сверху (Fraction of MTs):** количество ридов, сопоставленных с митохондриальными генами, деленное на общую сумму обработанных ридов.\
- **Слева снизу (Fraction of Genes):** количество ридов, сопоставленных с эндогенными генами, деленное на общую сумму обработанных ридов равно или больше 0,5.\
- **По центру снизу (Frac of Mapped Reads)**: количество ридов, которые сопоставлены с референсным геномом, деленное на общее количество ридов, которые были обработаны для каждой клетки равно или превышает 0,8. Это число автоматически предоставляется Salmon v0.8. 2.\
**- Справа снизу (Tot number of reads(Log10)):** log10 общего количества ридов равен или превышает 4.\
...
## Рис. 1 (d). UMAP-визуализация данных single-cell RNA-seq с клетками, окрашеннными в соответствие с условием среды, в которой они культивировались.

Клетки окрашены в соответствии с условием обработки ингибитором каспазы (CI) или обработки DMSO.

Cell competition OFF = CL-treated (367 cells)\
Cell competition ON = DMSO-treated (356 cells)
...
## Рис. 1 (c). UMAP-визуализация данных single-cell RNA-seq с клетками, окрашеннными в соответствие с кластером.

Для классификации клеток по кластерам исследователи провели иерархическую кластеризацию на матрице расстояний (hierarchical clustering on the distance matrix; функция «hclust» в R с методом агрегирования ward.D2) с последующим применением динамического гибридного алгоритма разрезания (функция «cutreeDynamic» в пакете R «dynamicTreeCut», с гибридным методом, минимальным размером кластера 35 клеток и параметром «deepSplit», равным 0), который выявил пять кластеров.

UMAP-визуализация данных single-cell RNA-seq, клетки окрашены в соответствии с кластером.

Идентичность пяти кластеров была установлена на основе экспрессии известных маркерных генов эпибласта, висцеральной энтодермы и экстраэмбриональной эктодермы, которые были идентифицированы в предыдущем исследовании [@scialdone2016]
...
В частности, один из этих кластеров (кластер 4, Loser Epiblast) состоит только из клеток эмбрионов, обработанных CI.

## Расширенные данные. Рис. 1 (f). DeepSplit кластеризация.

Изменения в составе и количестве кластеров между результатами кластеризации, полученными с помощью deepSplit 0 (вверху) и 1 (внизу), показаны с использованием библиотеки “clustree".

Пять кластеров:\
1) ExE (Extra Embrionic Ectoderm) - экстраэмбриональная эктодерма\
2) Intermediate - промежуточный кластер\
3) Los Epi (Loser Epiblast) - "проигравшие" эпибласты\
4) VE (Visceral Endoderm) - висцеральная эндодерма\
5) Win Epi (Normal (winner) Epiblast) - эпибласты-"победители"
...
## Расширенные данные. Рис. 1 (e). Робастность кластера.

Исследователи провели анализ робастности кластеризации, детально изучив, как выбор генов, параметров кластеризации и алгоритмов влияет на идентичность и количество кластеров.

Гамма Пирсона (левая панель) и средняя ширина силуэта (правая панель) были рассчитаны для каждого набора кластеров, полученных с помощью 100 случайных подвыборок из 60% высоко вариабельных генов и различных значений параметра deepSplit. Наиболее устойчивые кластеры соответствуют значениям deepSplit 0 и 1 (при deepSplit=0 и 1 робастность схожа), так как при больших значениях deepSplit (соответствующих менее консервативной кластеризации) робастность быстро снижается.
...
## Расширенные данные. Рис. 1 (d). UMAP-визуализация данных single-cell RNA-seq с клетками, окрашенными в зависимости от партии.

В каждой партии наблюдается сбалансированное распределение клеток по двум условиях (культивирование в присутствии или отсутствии CI) и по пяти кластерам.
...
## Рис. 1 (e). Отношение между количеством клеток эмбрионов, обработанных DMSO, и клеток, обработанных CI, в трех кластерах эпибласта (DMSO/CI). 

Если в кластере эпибласта «нормальный» наблюдается обогащение клетками из эмбрионов, обработанных DMSO, то в кластерах эпибласта «промежуточный» и «проигравший» наблюдается сильное обогащение клетками из эмбрионов, обработанных CI.
...
## Расширенные данные. Рис. 1 (b, c). Количество клеток хорошего качества в каждой партии зависимости от условий культивирования (b) или от принадлежности к кластеру (с).

Верхняя таблица (с) отражает количество качественных клеток в каждом кластере (строки) и партии (столбцы). \
Нижняя таблица (b) отражает количество качественных клеток в каждом условии (строки) и партии (столбцы).
...
## Рис. 1 (b). Идентификация кластеров в соответствии с известными генными маркерами из различных клеточных линий эмбриона.

В трех кластерах (кластеры Normal (winner) Epiblast, Intermediate, Loser Epiblast) представлены маркерные гены эпибласта (Epi), остальные кластеры соответствуют внеэмбриональным линиям висцеральной эндодермы (VE) и внеэмбриональной эктодермы (ExE). Кластеры эпибласта названы «Победитель», «Промежуточный» и «Проигравший» в зависимости от относительной доли клеток из эмбрионов, обработанных CI (см. Рис. 1 (e)).
...
# В клетках, уничтоженных во время эмбрионального развития мыши, выявлены транскрипционные маркеры клеточной конкуренции.

## Расширенные данные. Рис. 2 (c, d). Diffusion map-анализ всех клеток эпибласта (из эмбрионов, обработанных DMSO и CI). 

С помощью PAGA (библиотека python "scanpy") была произведена количественная оценка связности между кластерами. Анализ показал, что три кластера эпибластов связаны друг с другом, в то время как две экстраэмбриональные ткани (висцеральная эндодерма и экстраэмбриональная эктодерма) изолированны.

Диффузионная карта рассчитана с помощью функции “DiffusionMap” в пакете R “destiny” на основе данных анализа PAGA, для клеток эпибласта из эмбрионов, обработанных CI.

Координата псевдовремени была вычислена с помощью функции "DPT" для корневой клетки в кластере winner epiblast (определяемой функцией "tips" в пакете "destiny").

Клетки окрашены в соответствии с условием (c) и кластером (d).
...
## Рис. 2 (a, d). Диффузионная карта (diffusion map) клеток эпибласта (только из эмбрионов, обработанных CI), окрашенных по кластерам (а) или в соответствии с координатами псевдовремени диффузии (dpt, diffusion pseudotime).

Три кластера эпибластов тесно связаны, что подтверждается анализом взаимосвязей, проведенным с помощью PAGA (рис. 2б). Следовательно, чтобы установить взаимосвязь между этими кластерами эпибластов, необходимо рассчитать диффузионную карту. Для того, чтобы исключить возможные побочные эффекты, связанные с ингибитором каспазы, были отобраны только клетки, обработанные CI. Однако, если учесть все клетки эпибласта, результаты остаются неизменными (расширенные данные, рис. 2 (с-е)). Этот анализ выявил траекторию между тремя кластерами эпибластов. При этом клетки, присутствующие только в эмбрионах, обработанных CI, попали на один конец траектории, а клетки, присутствующие как в эмбрионах, обработанных DMSO, так и в эмбрионах, обработанных CI - попали на другой конец.

Кластеры победителей и проигравших находятся на двух концах пути (траектории), поэтому dpt можно интерпретировать как “результат проигрыша”.
...
## Рис. 2 (e). Losing score в трех кластерах эпибласта у эмбрионов, обработанных CI (слева) или DMSO (справа).

Исследователи оценили показатели "losing scores" эпибластных клеток из эмбрионов, обработанных DMSO, проецируя эти данные на ранее рассчитанную диффузионную карту (с использованием функции `dm_predict` из пакета `destiny`). Наконец, для каждой проецируемой клетки было присвоено значение "losing score" как среднее арифметическое значений "losing scores" 10 ближайших соседей на оригинальной диффузионной карте (определённых с помощью функции `projection-dist` из пакета `destiny`).

Оценка потерь клеток эмбрионов, обработанных DMSO, была получена путем их проецирования на карту диффузии, показанную в рис. 2 (d).
...
## Расширенные данные. Рис. 2 (e). Координата всевдовремени клеток эпибласта, обработанных CI.

Псевдовременная координата диффузии (diffusion pseudotime coordinate) клеток эпибласта, обработанных CI, полученная из диффузионной карты, включающей все клетки эпибласта, хорошо коррелирует с псевдовременной координатой, полученной из диффузионной карты, рассчитанной только для клеток эпибласта, обработанных CI.
...
## Рис. 2 (f). Уровни экспрессии генов-маркеров дифференциации клеток эпибластов эмбрионов, обработанных CI.

Среди них:\
- Гены-маркеры наивной плюрипотентности (Klf4, Klf5, Sox2, Pou3f1, Tcfl1, Pou5f1 и Rex1)\
- Гены-маркеры праймированной плюрипотентности (Fgf5 и Tdgf1)\
- Гены-маркеры мезодермы (Mesp1 и T)\
- Гены-маркеры нейроэктодермы (Neurod1 и Sox1)\
- Гены-маркеры энтодермы (Sox17 и Gata6)

Клетки (в столбцах) сортируются по "losing score". \
Гены, помеченные \*, экспрессируются по-разному вдоль траектории псевдовремени.
...
## Расширенные данные. Рис. 3 (d). Диаграмма рассеяния (Scatter plots) уравней экспрессии различных маркерных генов в "проигравших" эпибластах, сопоставленных друг с другом.

Klf4 (ген-маркер наивной плюрипотентности) vs Sox17 (ген-маркер энтодермы) - корреляция Спирмена = -0,127

Klf4 (ген-маркер наивной плюрипотентности) vs Neurod1 (ген-маркер нейроэнтодермы) - корреляция Спирмена = -0,108

Neurod1 (ген-маркер нейроэнтодермы) vs Sox17 (ген-маркер энтодермы) - корреляция Спирмена = 0,243

Клетки-"проигравшие" имеют более высокую экспрессию маркеров плюрипотентности, а также более высокую экспрессию некоторых других специфичных для клеточной линии маркеров, и совместная экспрессия этих маркеров слабо коррелирует.
...
## Модель GAM для выявления дифференциально экспрессируемых  генов вдоль псевдовремени, используя только данные эпибластных клеток, обработанных CI.

Для выявления генов с дифференциальной экспрессией вдоль траектории сначала были отобраны гены, которые имеют уровень экспрессии более 15 TPM в более чем 10 клетках (этот список генов представлен в Дополнительной таблице 4). Затем были получены логарифмически преобразованные уровни экспрессии этих генов (с добавлением 1 для избежания бесконечностей) в зависимости от значения "losing score", и к ним была применена обобщённая аддитивная модель (функция `gam` из пакета `GAM`, версия 1.16). Для оценки p-value для каждого тестируемого гена использовался параметрический ANOVA-тест, предоставляемый функцией `gam`.
...
## Выбор генов, которые изменяются значимым образом вдоль псевдовремени.

В результате был получен список из 5310 генов с дифференциальной экспрессией (FDR \< 0.01).
...
## Иерархический кластерный анализ (Hierarchical cluster analysis) для выявления генов, которые ведут себя сходным образом на протяжении всей траектории.

Для того, чтобы найти группы дифференциально экспрессируемых генов, которые демонстрируют схожие патерны экспрессии вдоль траектории, была рассчитана матрица расстояний на основе корреляции между генами по формуле: квадратный корень из выражения (1-p)/2, где p - коэффициент корреляции Спирмена между генами.

К этой матрице была применена иерархическая кластеризация (функция `hclust` с методом `ward.D2`), за которой последовал dynamic hybrid cut алгоритм (пакет `dynamicTreeCut`, функция `cutreeDynamic` с методом `hybrid`, минимальным размером кластера в 100 генов и параметром `deepSplit`, равным 0). Это позволило выделить четыре кластера, три из которых содержат гены с понижающейся экспрессией вдоль траектории (объединённые для анализа обогащения GO и IPA) и один кластер генов с увеличивающейся экспрессией.
...
## Идентификация дифференциально экспрессируемых генов (**up** — гены с повышающейся экспрессией вдоль траектории; **down** — гены с понижающейся экспрессией вдоль траектории)
...
## Расширенные данные. Рис. 3 (а). Тепловая карта (heatmap), отображающая паттерн экспрессии всех генов, дифференциально экспрессируемых вдоль траектории от выигравших к проигравшим клеткам (траектория представлена на рис. 2 d)

Чтобы дополнительно определить идентичность клеток эпибласта эмбрионов, обработанных CI, были проанализированны гены, по-разному экспрессирующиеся вдоль траектории, с помощью Ingenuity Pathway Analysis (IPA).

Важно отметить, что эти дифференциально экспрессируемые гены попали в категории молекулярных и клеточных функций, связанные с клеточной смертью и выживанием, синтезом белков и нуклеиновыми кислотами (рис. 2b).
...
## Расширенные данные. Рис. 3 (b, c). Перкрытие генов, дифференциально экспрессирующихся вдоль траектории, соединяющей "победившие" и "проигравшие" клетки эпибласта у эмбрионов, обработанных CI, и генов, на которые нацелен p53.

**up-regulated genes** — гены, экспрессия которых повышается в ответ на стимул (по сравнению с референсной экспрессией); \
**down-regulated genes** — гены, экспрессия которых понижается в ответ на стимул.

Круговые диаграммы показывают процент генов с повышающейся (up) или понижающейся (down) экспрессией в "проигравших" клетках среди группы таргетных генов, которые активируются (b) или репрессируются (c) p53.

Мишени RICTOR (компонент mTOR), MYC, MYCN и IGFR в основном попадают в гены с понижающейся экспрессией (дополнительные таблицы 1 и 2). При этом мишени, активируемые р53, преимущественно имели повышающуюся экспрессию, тогда как мишени, подавляемые р53, преимущественно демонстрировали понижающуюся экспрессию (расширенные данные, рис. 3b-c). Более того, было обнаружено, что гены, связанные с синтезом белка, в основном понижали свою экспрессию.
...
Обнаружение того, что дифференциально экспрессируемые гены связаны с биомаркерами клеточной смерти, а также являются мишенями mTOR, MYC и P53, убедительно указывает на то, что клетки на каждом конце траектории представляют собой «победителей» или «проигравших» клеточной конкуренции. В связи с этим эпибластные клетки, обнаруженные только в эмбрионах, обработанных CI, были названы «проигравшими» эпибластными клетками, а клетки на противоположном конце траектории — «победителями». Клетки, расположенные между этими популяциями на траектории, обозначены как «промежуточные». На основании этих данных была определена координата диффузионного псевдовремени (dpt), которая берет начало в кластере «победителей» и отслеживает положение клеток вдоль траектории. Эта координата интерпретируется как «losing score», то есть количественно отражает выраженность «проигравшего» состояния в транскриптоме клетки (рис. 2 d-e)

# У "проигравших" клеток наблюдаются нарушения в функционировании митохондрий

## Рис. 3 (с). и Расширенные данные, Рис. 4 (d). Диаграммы рассеяния (scatter plot) с уровнями экспрессии митохондриальных генов (Рис. 3 с), и генов, участвующих в реакции на стресс, в клетках эпибласта эмбрионов, обработанных CI (Рис. 4 d), в зависимости от losing score.

С помощью IPA были проанализированны клеточные пути, нарушенные у "проигравших" эпибластных клеток, и было обнаружено, что два ведущих пути — митохондриальная дисфункция и окислительное фосфорилирование — связаны с функцией митохондрий (рис. 3 a-b, Дополнительные таблицы 1 и 2). Например, было выявлено снижение экспрессии вдоль траектории от «победителей» к «проигравшим» таких генов митохондриальной ДНК, как **mt-Nd3** и **mt-Atp6**, регуляторов митохондриальной динамики, таких как **Opa1** (оптическая атрофия 1), а также генов, участвующих в организации митохондриальных мембран и крист, таких как **Samm50** (рис. 3 c). Эти данные свидетельствуют о том, что митохондриальная функция у проигравших клеток нарушена.

Современные исследования показывают, что стрессовые ответы, такие как интегрированный стрессовый ответ (ISR) или связанный с ним ответ на неправильно свернутые белки (UPR), активируемые в клетках с нарушенной митохондриальной функцией, инициируют транскрипционный программный механизм для восстановления клеточного гомеостаза. Мы обнаружили, что проигравшие эпибластные клетки демонстрируют характерную подпись UPR-ISR, а ключевые регуляторы этого ответа, такие как **Atf4**, **Ddit3**, **Nrf2** и **Foxo3**, имеют повышенную экспрессию в этих клетках (Дополнительные данные, рис. 4a-d). Аналогично, **Sesn2**, мишень p53, контролирующая активность mTOR, также демонстрировала повышенную экспрессию у проигравших клеток (Дополнительные данные, рис. 4d). Эти данные подтверждают, что у проигравших эпибластных клеток имеются митохондриальные дефекты, что приводит к активации стрессового ответа с целью восстановления клеточного гомеостаза.
...
## Расширенные данные. Рис. 4 (а). Перекрытие (overlap) генов, дифференциально экспрессирующихся вдоль траектории, соединяющей "выигравшие" и "проигравшие" клетки эпибласта в эмбрионах, обработанных CI (рис. 2 а и расширенные данные, рис. 3 а), и генов, связанных с путями UPR и ISR (расширенные данные, табл. 3)

Из 32 генов, относящихся к путям UPR и ISR, 12 генов демонстрируют пониженную экспрессию в проигравших клетках, 8 генов — повышенную, а 12 генов не проявляют дифференциальной экспрессии между проигравшими и выигравшими клетками. Было выявлено статистически значимое обогащение генов UPR_ISR среди генов с повышенной экспрессией в проигравших клетках (odds ratio = 3.0, p-value = 0.012). Пересечение между генами UPR_ISR и генами с пониженной экспрессией не является статистически значимым (odds ratio = 1.2, p-value = 0,69)
...
## Расширенные данные. Рис. 4 (b, c). Список генов из путей UPR-ISR, которые статистически значимо экспрессируются с повышением (b) или снижением (с) экспрессии у "проигравших" клеток.
...
## Расширенные данные. Рис. 10 (b). Пересечение генов, дифференциально экспрессируемых вдоль траектории от выигравших к проигравшим эпибластным клеткам ("in_vivo_scRNA-seq"; рис. 2 а и расширенные данные, рис. 3 а) и генов, дифференциально экспрессируемых между совместно культивируемыми ЭСК, HB (24%, "победители") и BG (95%, "проигравшие") (in vitro bulk RNA-seq)

Термины «Up» и «Down» здесь обозначают гены с повышенной или пониженной экспрессией у проигравших клеток.

Пересечение пониженно экспрессируемых генов (scRNA-seq in vivo) с пониженно экспрессируемыми генами (bulk RNA-seq in vitro): p-value = 1.71E-12; odds ratio = 1.80 - статистически значимый результат.

Пересечение пониженно экспрессируемых генов (scRNA-seq in vivo) с повышенно экспрессируемыми генами (bulk RNA-seq in vitro): p-value = 5.20Е-3; odds ratio = 0.67 - статистически значимый результат.

Пересечение повышенно экспрессируемых генов (scRNA-seq in vivo) с пониженно экспрессируемыми генами (bulk RNA-seq in vitro): p-value = 4.87Е-3; odds ratio = 0.80 - статистически значимый результат.

Пересечение повышенно экспрессируемых генов (scRNA-seq in vivo) с повышенно экспрессируемыми генами (bulk RNA-seq in vitro): p-value = 0.30; odds ratio = 1.14 - не является статистически значимым результатом.
...
## Расширенные данные. Рис. 10 (с). Пересечение значимо обогащенных терминов (enriched terms) в генах с повышенной или пониженной экспрессией в проигравших клетках эпибласта эмбрионов, обработанных CI ("in_vivo_scRNA-Seq"), и в in vitro модели клеточной конкуренции между совместно культивируемыми ЭСК c HB (24%, winner) и BG (95%, loser) ("in_vitro_bulk_RNA-seq")

Все термины, обогащённые среди генов с пониженной экспрессией **in vitro**, также обогащены **in vivo**.
...
## Сравнение с данными из работы Cheng, S. et al. **"[Single-Cell RNA-Seq Reveals Cellular Heterogeneity of Pluripotency Transition and X Chromosome Dynamics during Early Mouse Development](https://www.cell.com/cell-reports/fulltext/S2211-1247(19)30202-5)"**, *Cell Rep* 26, 2593-2607 e2593 (2019).
...
setwd("C:/Users/79143/Documents/kursovaya/kursovaya/Input_data_figures")
...
projec<-as.matrix(GetAssayData(data_pro,  layer="data",assay="RNA"))
...
## Клетки эпибласта на стадиях E5.5, E6.25 and E6.5 (Cheng et al)
...
## Расширенные данные. Рис. 3 (е, f). Данные scRNA-seq для эпибластных клеток были спроецированны на [ранее опубликованные данные](https://www.cell.com/cell-reports/fulltext/S2211-1247(19)30202-5) эпибластных клеток, полученных из только что изолированных эмбрионов на разных стадиях (Е5.5, Е6.25 и Е6.5)

Для эпибластных клеток из свежеизвлеченных эмбрионов были рассчитаны диффузионная карта (e) и псевдовремя (f). Затем для данных из основной статьи была оценена координата псевдовремени после их проекции на диффузионную карту.
...
## Проекция эпибластных клеток, культивируемых без CI (в присутствии DMSO)
...
## Расширенные данные. Рис. 3 (g). Координаты псевдовремени для обеих выборок, разделённые по стадиям (E5.5; E6.25; E6.5), обработке (DMSO/CI) и кластеру (Winner Epiblast; Loser Epiblast; Intermediate).
...
## Расширенные данные. Рис. 2 (а). Анализ клеточного цикла эпибластов из кластеров 1,3,4 (Intermediate Epi - CI; Los Epi - CI; Win Epi - CI)

Фаза клеточного цикла была предсказана с помощью алгоритма cyclone algorithm58 и показывает, что клетки в фазе S и G2M есть в проигравшем и в промежуточном кластерах.
...
Наконец, используя обобщенные аддитивные модели (generalized additive models), исследователи определили позиции, гетероплазмия H которых статистически значимым образом изменяется в зависимости от количества проигравших клеток. Было обнаружено в общей сложности одиннадцать значимых позиций (FDR \< 0,001), шесть из них в гене mt-Rnr1 и пять в гене mt-Rnr2. Все эти позиции имеют более высокий уровень гетероплазмии в проигравших клетках (см. рис. 6b-g и расширенные данные рис. 7f-k). Результаты остаются практически неизменными, если использовать тест ранговой корреляции Спирмена (в качестве альтернативы обобщенным аддитивным моделям). Наиболее распространенными заменами, наблюдаемыми в каждой позиции, являются следующие: mt-Rnr1 300 A-to-C; mt-Rnr1 303 T-to-G; mt-Rnr1 304 T-to-G; mt-Rnr1 305 C-to-G;mt-Rnr1 326 A-to-G; mt-Rnr1 327 C-to-G; mt-Rnr2 2031 T-to-G; mt-Rnr2 2074 C-to-G; mt-Rnr2 2077 A-to-C; mt-Rnr2 2079 C-to-T; mt-Rnr2 2081 A-to-Г.
...
## Только обработанные CI клетки эпибласта
...
## Расширенные данные. Рис. 7 (m). Схематическое представление митохондриального генома, на котором красным цветом обозначены участки, прошедшие фильтрацию по покрытию и учтенные в анализе гетероплазмии. 

Указаны только те гены, которые находятся на этих участках.
...
## Рис. 6 (b-g) и расширенные данные, рис. 7 (a-e). Гетероплазмия мтДНК в эпибластных клетках эмбрионов, обработанных CI, для шести позиций в гене mt-Rnr1(326, 327, 305, 304, 300, 303) и пяти позиций в гене mt-Rnr2 (2077, 2079, 2081, 2031, 2074) 

Гетероплазмия = 1 - частота наиболее распространённого аллеля.

Во всех этих позициях наблюдается статистически значимое увеличение гетероплазмии с ростом "losing score" — откорректированные p-значения указаны в верхней части каждого графика.
...
## Рис. 6 (а). Средняя гетероплазмия мтДНК с учетом всех 11 полиморфизмов, которые имеют статистически значимую зависимость от "losing score".
...
## Расширенные данные. Рис. 7 (f-k). Вариация гетероплазмии среди клеток, обработанных CI, для 6 значимых позиций в гене mt-Rnr1.

Данные показывают, что вариация гетероплазмии не обусловлена особенностью партии (помета) эмбриона:

Число клеток, проанализированных по кластерам (и партиям), следующее: \
число клеток в нормальном эпибласте: 42 (1), 16 (2), 18 (3), 0 (4), 2 (5); \
число клеток в промежуточном эпибласте: 42 (1), 28 (2), 28 (3), 12 (4), 5 (5); \
число клеток в эпибласте "проигравших": 22 (1), 15 (2), 20 (3), 2 (4), 7 (5).
...
## Рис. 6 (i). Коэффициент корреляции Спирмена между гетероплазмией мтДНК в шести позициях mt-Rnr1.
...
## Расширенные данные. Рис. 7 (i). Корреляция гетероплазмии мтДНК на всех статистически значимых позициях, включая шесть позиций в пределах гена mt-Rnr1 и пять позиций в пределах гена mt-Rnr2

Стоит отметить, что изменения последовательности, которые были обнаружены в mt-Rnr1 и mt-Rnr2, сильно соотносились в одних и тех же клетках, причем те, которые находятся ближе друг к другу, имели наибольшую вероятность существования одновременно.

Это свидетельствует о возможных ошибках репликации мтДНК, которые могут «повреждать» мтДНК, нарушая функцию mt-Rnr1 (12S рРНК) и mt-Rnr2 (16S рРНК), что приводит к фенотипу «проигравших». Важно, что присутствие этих специфических мутаций мтДНК в клетках «проигравших» предполагает, что клеточная конкуренция может способствовать устранению вредоносных мутаций мтДНК в ходе раннего развития мыши. Следует отметить, что мы сообщаем только о вариантах мтДНК, обнаруженных в областях генома с высоким покрытием секвенирования (Расширенные данные Рис. 7m), поэтому наличие других вариаций в последовательности мтДНК между клетками «победителей» и «проигравших» не может быть исключено.
...
## Рис. 6 (h). Столбчатая диаграмма (bar plot), показывающая долю клеток эпибласта в каждом из кластеров на оси Х (победитель, промежуточный, проигравший), которые имеют среднюю гетероплазмию (вычисленную по шести позициям в пределах гена mt-Rnr1, указанным в b-g) выше 0,01.

Для построения столбчатой диаграммы были учитены только те клетки, которые охватывали более чем 50 считываниями все значимые позиции в гене mt-Rnr1 (215 клеток, рис. 6h-6i) или в обоих генах mt-Rnr1 и mt-Rnr2 (214 клеток, рис. 7h-6i).. 7l).
